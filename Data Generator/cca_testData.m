function X = cca_testData(N,FLAG)
%-------------------------------------------------------------------
...
elseif FLAG == 6   %SBL—LAP论文仿真
    
    N = N + settleTime;
    X = cca_normrnd(0,1,6,N);   % 6 variables 
    for i=4:N
        X(1,i) = X(1,i) + 0.22.*X(1,i-1) + 0.56.*X(4,i-2);
        X(2,i) = X(2,i) - 0.22.*X(2,i-1) + 0.55.*X(1,i-1);
        X(3,i) = X(3,i) + 0.48.*X(2,i-3);
        X(4,i) = X(4,i) + 0.51.*X(1,i-2) + 0.85.*X(3,i-3);
        X(5,i) = X(5,i) + 0.42.*X(4,i-1) + 0.40.*X(6,i-2);
        X(6,i) = X(6,i) + 0.65.*X(1,i-2);
    end
    X = X(:,settleTime+1:end);
...
elseif FLAG == 7
    ...
    X = cca_normrnd(0,1,5,N);   % 5 variables 
    for i=4:N
        X(1,i) = X(1,i) + 0.95.*sqrt(2).*X(1,i-1) - 0.9025.*X(1,i-2);
        X(2,i) = X(2,i) + 0.5.*X(1,i-2) - 0.4.*X(3,i-2);
        X(3,i) = X(3,i) - 0.4.*X(1,i-3) + 0.4.*X(2,i-2);
        X(4,i) = X(4,i) - 0.5.*X(1,i-2) + 0.25.*sqrt(2).*X(4,i-1) + 0.25.*sqrt(2).*X(5,i-1);
        X(5,i) = X(5,i) - 0.25.*sqrt(2).*X(4,i-1) + 0.25.*sqrt(2).*X(5,i-1);
    end
    X = X(:,settleTime+1:end);
...
elseif FLAG == 8
    ...
    N = N + settleTime;
    X = cca_normrnd(0,1,8,N);   % 8 variables
    for i=7:N
        X(1,i) = X(1,i) + 0.4.*X(8,i-1) - 0.55.*X(8,i-2) + 0.3.*X(1,i-1) + 0.4.*X(8,i-3);
        X(2,i) = X(2,i) + 0.5.*X(1,i-1) + 0.6.*X(2,i-1) + 0.4.*X(2,i-2);
        X(3,i) = X(3,i) - 0.4.*X(4,i-3) + 0.7.*X(4,i-2) + 0.5.*X(4,i-1) - 0.25.*X(3,i-3) + 0.25.*X(3,i-1);
        X(4,i) = X(4,i) - 0.5.*X(5,i-2) + 0.25.*X(8,i-2) + 0.32.*X(8,i-1) + 0.25.*X(5,i-1) + 0.5.*X(8,i-3);
        X(5,i) = X(5,i) - 0.25.*X(5,i-2) + 0.25.*X(6,i-1) - 0.5.*X(6,i-2) + 0.25.*X(5,i-1);
        X(6,i) = X(6,i) + 0.25.*X(5,i-2) + 0.25.*X(7,i-2) + 0.41.*X(7,i-1) - 0.25.*X(5,i-1) + 0.25.*X(7,i-3);
        X(7,i) = X(7,i) - 0.5.*X(8,i-3) + 0.25.*X(8,i-2) + 0.31.*X(8,i-1) - 0.5.*X(7,i-2) + 0.2.*X(7,i-1);
        X(8,i) = X(8,i) - 0.25.*X(8,i-6) + 0.25.*X(8,i-4) + 0.33.*X(8,i-3) + 0.25.*X(8,i-2) + 0.25.*X(8,i-1);
%   
    end
    X = X(:,settleTime+1:end);
    

elseif FLAG == 9    %自己创立的仿真模型
    
    N = N + settleTime;
    X = cca_normrnd(0,1,6,N);   % 6 variables 
    for i=4:N
        X(1,i) = X(1,i) + 0.30.*X(1,i-1) + 0.67.*X(4,i-2);
        X(2,i) = X(2,i) - 0.50.*X(2,i-1) + 0.80.*X(1,i-1) + 0.70.*X(3,i-3);
        X(3,i) = X(3,i) + 0.45.*X(2,i-3) + 0.31.*X(1,i-2);
        X(4,i) = X(4,i) + 0.71.*X(1,i-2) + 0.57.*X(3,i-3);
        X(5,i) = X(5,i) + 0.83.*X(4,i-1) + 0.61.*X(6,i-2) + 0.73.*X(1,i-3);
        X(6,i) = X(6,i) + 0.49.*X(1,i-2);
%   
    end
    X = X(:,settleTime+1:end);


elseif FLAG == 10    %自己创立的仿真模型（2）
    
    N = N + settleTime;
    X = cca_normrnd(0,1,6,N);   % 6 variables 
    for i=4:N
        X(1,i) = X(1,i) + 0.30.*X(1,i-1) + 0.67.*X(5,i-2);
        X(2,i) = X(2,i) - 0.50.*X(2,i-1) + 0.80.*X(1,i-1) + 0.70.*X(3,i-3);
        X(3,i) = X(3,i) + 0.45.*X(2,i-3);
        X(4,i) = X(4,i) + 0.57.*X(3,i-3);
        X(5,i) = X(5,i) + 0.83.*X(4,i-1) + 0.61.*X(6,i-2);
        X(6,i) = X(6,i) + 0.49.*X(1,i-2);
%   
    end
    X = X(:,settleTime+1:end);
...
elseif FLAG == 11    %自己创立的仿真模型（3）
    ...
    for i=4:N
        ...
        X(2,i) = X(2,i) - 0.50.*X(2,i-1) + 0.80.*X(1,i-1) + 0.70.*X(3,i-3);
        X(3,i) = X(3,i) + 0.45.*X(2,i-3);
        X(4,i) = X(4,i) + 0.57.*X(3,i-3) + 0.79.*X(2,i-1);
        X(5,i) = X(5,i) + 0.83.*X(4,i-1) + 0.61.*X(6,i-2);
        X(6,i) = X(6,i) + 0.49.*X(1,i-2);
%   
    end
