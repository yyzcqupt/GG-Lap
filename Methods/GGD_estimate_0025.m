function [X] = GGD_estimate_0025(Y,A)
...
%     cost = T*log(det(Gamma + A'*pinv(Beta)*A)) + 2*T*n*log(lamda) + lamda^2*trace(Beta) + trace((Y - A*X)'*pinv(Beta)*(Y - A*X)) + trace(X'*Gamma*X) - sum((p-2)*alpha^(-p)*rr.^p) + 2*size(X,1)*log(alpha);
    cost = T*log(det(Gamma + A'*pinv(Beta)*A)) - 2*T*n*log(lamda) + lamda^2*trace(Beta) + trace((Y - A*X)'*pinv(Beta)*(Y - A*X)) + trace(X'*Gamma*X) - sum((p-2)*alpha^(-p)*rr.^p) + 2*size(X,1)*log(alpha);
    % print
    disp(['GGD_estimate_0025_iters: ',num2str(count),'   cost',num2str(cost)]);
    disp(['GGD_estimate_0025_iters: ',num2str(count),'   cost change: ',num2str(abs((cost-cost_old)/cost_old))]);
