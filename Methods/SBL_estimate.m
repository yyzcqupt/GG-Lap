function [x] = SBL_estimate(Y,A, lambda, Learn_Lambda, varargin)
...
while (1)
    ...
    if (prune_hyperparameter )
        ...
        index = find(gamma > max(gamma)*PRUNE_GAMMA); %prue-gamma的方式修改为根据gamma相对值prune
    ...
    Sigma_w_diag = real( gamma - (sum(Xi'.*(A*Gamma)))');
    ...
    if Learn_Lambda == 1
        lambda = (norm(Y - A * mu,'fro')^2/L)/(N-m + sum(Sigma_w_diag./gamma_old)); 
    end
