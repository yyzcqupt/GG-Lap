...
% get input argument values 获取输入参数值
...
%% S update
% S_inter_old = zeros(Nsource,T);
% for Siter = 1:20
%     P = V'*(repmat(w,1,Nsource).*V); P = P + 1e-9*trace(P)/Nsource*speye(Nsource);
%     ivPLT = P\L';
%     S = ivPLT/(eye(Nsensor) + L * ivPLT)*B;
%     
% %     S = zeros(Nsource,size(B,2));
% %     for i = 1:size(LtB,2)
% %         S(:,i) = linsolve_lcg(V,w,L,1,LtB(:,i));
% %     end
%     
%     U = V*S;
%     w = lam^(-p)*(kesi+sum(U.^2,2)).^(p/2-1);
% %     if  norm((S - S_inter_old),'fro')/norm(S,'fro') < eps || sum(w) == 0
% %         break;
% %     end
% %     S_inter_old = S;
% end
...
%     P = V'*(repmat(gamma,1,Nsource).*V); P = P + 1e-9*trace(P)/Nsource*speye(Nsource);    
%     ivPLT = P\L';
%     S = ivPLT/(eye(Nsensor) + L * ivPLT)*B;
    
%     S = zeros(Nsource,size(B,2));
%     for i = 1:size(LtB,2)
%        S(:,i) = linsolve_full(L,1,V,gamma,LtB(:,i));
%     end
...
% % opts.outerMethod = 'woodbury';
% opts.innerOutput = 1;
% opts.outerOutput = 1;
% opts.outerNiter = 50;                           % Number of outer loop iterations
% opts.outerZinit = 2*1e-8;%1e-14;%sqrt(eps);%1e-14;
% opts.outerVarOpts = struct('MVM',180,                    ... % lanczos variances
%                            'NSamples',20,'Ncg',20);           % sample variances
% opts.innerIt = 15;                       
% opts.innerMVM   = 150;        % Number of CG steps in inner loop
% opts.innerVBpls = 'plsTN';%'plsLBFGS'; % PLS algorithm for inner loop
% opts.sparse = 0.15;%0.67;
% % opts.laplacian = 0.8;
% opts.innerType = 'VB';
% opts.variation = 'Sparse+Variation';
% opts.output = 1;
% 
% s = solve_S(B,L,p,kesi,S,V,lam,opts);
% S = reshape(s,[T,Nsource])';
...
rr = sqrt(sum(VS.^2,2)+kesi);
% lam = (p*sum(rr.^p)/size(V,1))^(1/p);
% nEdge = size(V,1); 
% fun = @(p) nEdge/p + psi(1/p)*nEdge/p^2 -sum(log(rr/lam).*(rr/lam).^p); 
% p = fsolve(fun,0.1)
% p = min(1,p);
